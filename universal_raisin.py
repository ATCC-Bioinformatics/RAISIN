...
positions = anchor_genomes(anchor, strain, sample_consensus)
...
def anchor_genomes(anchor_sequence, strain_sequence, consen_sequence):
    ...
    for i in range(len(anchor_sequence)):
        ...
        if a_nuc == "-":
            ...
            #! this might not work if the deletion of anchor nuc is at the end of genome
            # we need to someway to tell that the insertion is done (that's when we'll add it to the dictionary)
            # check the next nucleotide, if that is not a dash, then the insertion is over
